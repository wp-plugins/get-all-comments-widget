...
class CommentsSite extends WP_Widget {
	...
	function widget($args, $instance) {
		...
		foreach ($sites as $blog){
			...
			foreach($comments as $comment){
				...
				} ?> bei <a href="<?php echo get_blog_permalink($comment->blog_id, $comment->ID);?>"> <?php echo $comment->post_title; ?></a>
				</li>
				<?php
	...
	function form($instance) {
		...
		<p><label for="<?php echo $this->get_field_id('count'); ?>">Anzahl der Kommentare: <input class="widefat" id="<?php echo $this->get_field_id('count'); ?>" name="<?php echo $this->get_field_name('count'); ?>" type="text" value="<?php echo attribute_escape($count); ?>" /></label></p>
		<p><small><a href="http://kanedo.net">kanedo.net</a></small></p>
		<?php
	}
}
